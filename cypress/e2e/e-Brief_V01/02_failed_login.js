...
describe("Failed Login to E-Brief", () => {
  ...
  it('Failed Login - Login form is an empty', function(){
    ...
    cy.url().should("include", "https://kiamabn.b2clogin.com/kiamabn.onmicrosoft.com"); // Validation kiam url
    cy.wait(1000);
  ...
  it.only('Failed to login - invalid un/pw', function(){
    cy.visit('/');
    cy.url().should("include", "https://www.e-brief.at/fe_t"); // => validate url
    cy.get('#onetrust-accept-btn-handler').click();//remove cookie bar
    cy.wait(1000);
    ...
    const invalid_username = "INVALID_USERNAME___" + randomString + "@gmail.com";
    const invalid_password = "INVALID_PASSWORD___" + randomString;
    //Enter invalid credentials (un/pw)
    cy
    .get('#signInName').type(invalid_username)
    .get('#password').type(invalid_password)
    .get('#showPassword').click()//Show/Hide pass
    .wait(1000);
    ...
    cy.get('#kiam-login-failed > .userlogin__headline').should('have.text','Login fehlgeschlagen');//Verify Error message
    cy.get('#kiam-login-failed-subline-1').should('have.text','Bitte versuchen Sie es nochmals!');
    }) 
});
